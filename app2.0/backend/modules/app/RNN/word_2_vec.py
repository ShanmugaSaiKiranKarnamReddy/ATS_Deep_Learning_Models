...
from .predictrnn import get_rnn_result
...
def normalized_model_score(results):
    ...
    return [{'filename': getUser(k), 'result':round(statistics.mean(v), 2), 'data': v} for k,v in tmp.items()]

    


def final_result_rnn(session_id):
    documents=[]
    ...
    result = get_rnn_result(random_pairs)
    
    return normalized_model_score(result)
